...
public class HeartRateDecoView extends View {
     ...
     * 底行文字
     */
    private String bottomLineText = "bpm";
    ...
    public HeartRateDecoView(Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
        init(context);
    }

    public HeartRateDecoView(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
    ...
    public void setValue(float value1) {
        if ((int)value1 == this.value1){
            return;
        }
        this.value1 = (int) value1;
        setData(value1/maxValue);
    }
    ...
    private void setData(final float value1) {
...
//        animator = ValueAnimator.ofFloat(0, 1);
        ...
        Color.colorToHSV(barColor, hsv2);
    ...
    private void drawText(Canvas canvas) {
        ...
        mainTextPaint.setTextAlign(Paint.Align.CENTER);
        canvas.drawText(displayNumber1+"",x,y-12,mainTextPaint);
        int textPadding = ViewUtils.dp2px(density,16);
        canvas.drawText(getContext().getString(R.string.heart_rate),x, width / 2 - textHeight / 2 - textPadding,otherTextPaint);
