    ...
    public boolean dispatchTouchEvent(MotionEvent event) {
        switch (event.getAction()) {
            case MotionEvent.ACTION_DOWN: {
                ...
                slidePosition = getChildLayoutPosition(itemView);
                // 无效的position, 不做任何处理
    ...
    private void scrollByDistanceX() {
        ...
        if (orientation == Orientation.VERTICAL) {
            if (itemView.getScrollX() >= screenWidth / 2) {
                scrollLeft();
//            } else if (itemView.getScrollX() <= -screenWidth / introductor2) {
//                scrollRight();
            } else {
            ...
            if (itemView.getScrollY() >= screenWidth / 2) {
                scrollLeft();
//            } else if (itemView.getScrollY() <= -screenWidth / introductor2) {
//                scrollRight();
            } else {
                // 滚回到原始位置,为了偷下懒这里是直接调用scrollTo滚动
                itemView.scrollTo(0, 0);
        ...
        public static Orientation valueOf(int i) {
            switch (i) {
                ...
                case 1:
                    return VERTICAL;
                default:
                    throw new RuntimeException("[0->HORIZONTAL, introductor1->VERTICAL]");
            }
        }
    }
}
