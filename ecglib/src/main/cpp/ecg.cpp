...
JNIEXPORT jdoubleArray JNICALL
    Java_com_mgtech_ecglib_EcgUtil_filter(JNIEnv *env, jclass type, jdoubleArray ecg_data, jint ecgSize,
                                          jdouble sampleRate) {
...
//        __android_log_print(ANDROID_LOG_INFO, "JNITag","add size : %d", ecgSize);
    ...
    Java_com_mgtech_ecglib_EcgUtil_detect(JNIEnv *env, jclass type, jdoubleArray ecg_data,
    ...
    Java_com_mgtech_ecglib_EcgUtil_stringFromJNI(JNIEnv *env, jclass type) {
        std::string hello = "Ecg String from JNI";
        return env->NewStringUTF(hello.c_str());
    }

    JNIEXPORT jdoubleArray JNICALL
    Java_com_mgtech_ecglib_EcgUtil_detectEcg(JNIEnv *env, jclass type, jdoubleArray ecg_data,
                                             jint ecgSize, jdouble sampleRate, jdouble hrMin,
                                             jdouble hrMax, jdouble rr_num) {
        jdouble *ecgData = env->GetDoubleArrayElements(ecg_data, NULL);
