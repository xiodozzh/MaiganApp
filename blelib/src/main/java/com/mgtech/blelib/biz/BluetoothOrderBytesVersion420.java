...
public class BluetoothOrderBytesVersion420 extends BluetoothOrderBytes {
    private static final String TAG = "OrderBytes420";

    public BluetoothOrderBytesVersion420(IBraceletInfoManager manager) {
        super(manager);
    }
            ...
            for (int index : list) {
                ...
                switch (index) {
                    case SystemParamData.PARAM_REMINDERS:
                        ...
                        code = new byte[3 + size * 6];
                        code[0] = SystemParamData.PARAM_REMINDERS;
                        code[1] = 1;
                        code[2] = (byte) size;
                        for (int i = 0; i < size; i++) {
                            AlertReminder item = reminders.get(i);
                            code[3 + i * 6] = (byte) (item.isReminderEnable() ? 0x01 : 0x00);
                            code[4 + i * 6] = (byte) item.getReminderWeek();
                            code[5 + i * 6] = (byte) item.getReminderStartHour();
                            code[6 + i * 6] = (byte) item.getReminderStartMinute();
                            code[7 + i * 6] = (byte) (item.getReminderSpanTime() & 0xFF);
                            code[8 + i * 6] = (byte) ((item.getReminderSpanTime() >> 8) & 0xFF);
                        }
                        break;
