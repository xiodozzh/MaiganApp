...
public class PreferenceUtil {
    ...
    public static void writeStringToPreferences(Context context, String key,
        ...
        if (encrypt) {
            try {
                byte[] result = AESUtils.encrypt(BleConstant.PREFE_KEY, value.getBytes(CHARSET2), BleConstant.PREFE_VECTOR);
                if (result == null) {
                    return;
                }
//                String saveThis = Base64.encodeToString(array, Base64.DEFAULT);
    ...
    public static boolean safeWriteBytesToPreferences(Context context, String key,
        ...
        if (encrypt) {
            try {
                byte[] result = AESUtils.encrypt(BleConstant.PREFE_KEY, value, BleConstant.PREFE_VECTOR);
                if (result == null) {
                    return false;
                }
                editor.putString(key, byteToString(result));
    ...
    public static byte[] getBytesFromPreferences(Context context, String key) {
        ...
        if (encrypt) {
            try {
                ...
                return AESUtils.decrypt(BleConstant.PREFE_KEY, getBytes(s), BleConstant.PREFE_VECTOR);
            } catch (Exception e) {
                e.printStackTrace();
                return null;
            }
